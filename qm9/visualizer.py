...
def list2pkl(one_hot, charges, positions, dataset_info,node_mask=None):
    ...
    positions = np.round(positions, 5)
    for batch_i in range(one_hot.size(0)):
        mol = {}
        atoms = torch.argmax(one_hot[batch_i], dim=1)
        n_atoms = int(atomsxmol[batch_i])
        mol["natoms"] = n_atoms
        mol["elements"] = [ dataset_info['atom_decoder'][atoms[i]] for i in range(0,n_atoms)]
        coordinates = []
        for i in range(0,n_atoms):
            coordinate = positions[batch_i,i].tolist()
            coordinate = [round(j,5) for j in coordinate]
...
def plot_data3d(positions, atom_type, dataset_info, camera_elev=0, camera_azim=0, save_path=None, spheres_3d=False,
    ...
    if bg == 'black':
        ax.xaxis.pane.set_edgecolor("black")  # 修改 x 轴边框颜色
        ax.yaxis.pane.set_edgecolor("black")  # 修改 y 轴边框颜色
        ax.zaxis.pane.set_edgecolor("black")  # 修改 z 轴边框颜色
    else:
        ax.xaxis.pane.set_edgecolor("white")  # 修改 x 轴边框颜色
        ax.yaxis.pane.set_edgecolor("white")  # 修改 y 轴边框颜色
        ax.zaxis.pane.set_edgecolor("white")  # 修改 z 轴边框颜色
...
def plot_data3d_uncertainty(
    ...
    if 'qm9' in dataset_info['name']:
        ...
        ax.set_ylim(-axis_lim, axis_lim)
        ax.set_zlim(-axis_lim, axis_lim)
    elif dataset_info['name'] == 'geom' or  dataset_info['name'] == 'competition':
        max_value = all_positions[0].abs().max().item()
