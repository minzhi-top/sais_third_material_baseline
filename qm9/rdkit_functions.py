...
def retrieve_qm9_smiles(dataset_info):
    ...
    file_name = 'data/competition/competition_smiles.pickle'
    try:
...
class BasicMolecularMetrics(object):
    ...
    def evaluate(self, generated,test =False):
        ...
        if test:
            stable = generated 
            if len(stable) > 0:
                unique, uniqueness = self.compute_uniqueness(stable)
                print(f"Uniqueness over {len(stable)} valid molecules: {uniqueness * 100 :.2f}%")
                ...
                uniqueness = 0.0
                unique = None
            return [len(stable), uniqueness, novelty], unique
        else:
