...
competition_with_h = {
    'name': 'qm9',
    'atom_encoder': {'H': 0, 'C': 1, 'N': 2, 'O': 3, 'F': 4,'P': 5, 'S': 6, 'Cl': 7, 'Br': 8},
    'atom_decoder': ['H', 'C', 'N', 'O', 'F','P','S','Cl','Br'],
    'n_nodes': {21: 2705, 25: 2464, 36: 584, 27: 2145, 9: 265, 20: 2631, 32: 1143, 14: 1519, 29: 1777, 10: 399, 
    ...
    'max_n_nodes': 60,
    'atom_types':{0: 143905848, 1: 129988623, 2: 20266722, 3: 21669359, 4: 1481844, 5: 36290, 6: 3999872, 7: 1224394, 8: 298702},
    'distances': [903054, 307308, 111994, 57474, 40384, 29170, 47152, 414344, 2202212, 573726,
                  1490786, 2970978, 756818, 969276, 489242, 1265402, 4587994, 3187130, 2454868, 2647422,
                  2098884,
                  2001974, 1625206, 1754172, 1620830, 1710042, 2133746, 1852492, 1415318, 1421064, 1223156,
...
def get_dataset_info(dataset_name, remove_h):
    if dataset_name == 'qm9':
        ...
        if not remove_h:
            ...
            raise Exception('Missing config for %s without hydrogens' % dataset_name)
    elif dataset_name == "competition":
        if not remove_h:
            return competition_with_h 
        else:
            return  Exception('This is not be permited')
    else:
